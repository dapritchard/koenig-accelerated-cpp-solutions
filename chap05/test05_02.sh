...
# program location
exer_loc="exer05_02_vector"

# process command line arguments and define testing functions
. ../testing/system_test.sh

# we've checked for `exer05_02_vector', now check for `exer05_02_list'
if [[ ! -x exer05_02_list ]]; then
...
actual=$(./exer05_02_vector <<< "$input")
ST_ASSERT_EQUAL "$target" "$actual"

# test list-based version
actual=$(./exer05_02_list <<< "$input")
ST_ASSERT_EQUAL "$target" "$actual"
